    ...
    >
      <div className="grid md:grid-cols-[1fr_auto] items-center gap-6">
        <div>
          ...
          {cleanBody && (
            <p className={`mt-3 text-sm md:text-base leading-relaxed ${bodyClass}`}>
              {cleanBody}
            </p>
          )}
        </div>
        {cleanCtaLabel && (
          <Link
            ...
            className={`inline-flex items-center px-6 py-3 rounded font-medium transition-all duration-200 ease-out motion-reduce:transition-colors active:scale-[0.98] motion-reduce:active:scale-100 self-start md:self-center ${btn}`}
          >
            {cleanCtaLabel}
          </Link>
        )}
